...
test("data filters, refreshed pagination, varietal statistics and atomic edit versions preserve records", async ({ request, page }) => {
  ...
  try {
    ...
    const legacy = await localFixtureRpc(client, "create_bean_record", { p_bean: { ...base, name: "Legacy Geisha", varietal: "게이샤" }, p_tags: [], p_components: [] });
...
test("legacy whitespace and stale blend headers cannot create unselectable or misleading filters", async ({ request, page }) => {
  ...
  try {
    ...
    const single = await localFixtureRpc(client, "create_bean_record", { p_bean: base, p_tags: [], p_components: [] });
    ...
    const bom = await localFixtureRpc(client, "create_bean_record", { p_bean: { ...base, name: "Legacy BOM record", roastery: "\ufeffQA BOM\ufeff", varietal: "\ufeffGeisha\ufeff" }, p_tags: [], p_components: [] });
