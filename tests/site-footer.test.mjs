...
test("the shared footer separates itself without an upper rule", () => {
  const classes = footer.match(/<footer className="([^"]+)"/)?.[1] ?? "";
  assert.match(classes, /(?:^|\s)bg-surface(?:\s|$)/);
  assert.doesNotMatch(classes, /(?:^|\s)border(?:-[^\s]+)?(?:\s|$)/);
});
