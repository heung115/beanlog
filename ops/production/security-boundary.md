# 2026-09-07 보안 감사: 컨테이너 격리 운영 절차

이 문서는 컨테이너 격리의 운영 적용·검증·복구 절차다. 저장소 변경만으로 호스트
설정이 바뀌지는 않는다. 2026-09-07 실제 운영에서는 별도 DB 복원과 Auth 통합 검증
후 DB·인증·관리 서비스의 제한 및 관리망 경계를 적용했다. 앱 재배포 뒤에도 아래
완료 판정을 다시 확인한다. 이후 이미지나 네트워크 변경도 같은 검증을 거친다.
...
- `DOCKER-USER` 첫 규칙은 관리 bridge로 들어오는 다른 인터페이스의 전달 트래픽을
  ...
  호스트 Caddy는 Studio의 고정 관리망 IPv4로 직접 연결한다. 이 연결은 호스트 OUTPUT
  경로라 유지된다. internal 전용 망의 Studio에는 host port를 게시하지 않는다.
- IPv4 전달 트래픽의 `169.254.169.254/32` 목적지는 DNS TCP/UDP 53만 예외로 두고
  차단하여 Docker bridge 컨테이너의 OCI IMDS HTTP/HTTPS 접근을 막는다. OCI는
  같은 IP에서 DNS도 제공하므로 해당 포트만 기존 Docker 정책으로 돌려보낸다.
  운영 embedded DNS는 예외 추가 전에도 정상이었고, 예외는 직접 resolver 사용과
  빌드 환경의 호환성을 유지한다. 호스트의 IMDS 접근은 바꾸지 않는다.
...
- OCI dynamic group/Instance Principal 권한과 일반 egress 목적지 허용목록은 별도
  운영 검토 범위다. Tailscale 전용 SSH 호스트 규칙은 IPv4/IPv6에 적용했으며,
  독립 SSH 접속 확인 후 임시 자동 복구 장치를 해제했다.
