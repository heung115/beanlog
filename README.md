...
평점·감상·테이스팅 태그는 자동으로 바뀌지 않습니다. 구성별 국가와 비율이 명시되고 합이
100%인 블렌드는 같은 나라의 서로 다른 원두도 구분해 보여줍니다. 블렌드 구성은 직접 체크해야
종류와 구성이 바뀌며 불완전한 비율을 임의 보정하지 않습니다. 혼합 가공은 별도 확인 후보로 제시합니다.
...
로고·배경이 큰 사진은 픽셀의 작은 글자 행을 찾아 전체 사진과 본문을 따로 읽습니다.
