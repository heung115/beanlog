    ...
    <div className="mx-auto max-w-5xl pb-10 md:pb-16">
      ...
      <div className="mt-10 space-y-14 md:mt-14 md:space-y-20">
        {originGroups.map((group, groupIndex) => {
          ...
          return (
            <section id={group.id} key={group.id} aria-labelledby={`${group.id}-title`} className="scroll-mt-44 md:grid md:grid-cols-[9rem_minmax(0,1fr)] md:gap-10">
                  ...
                  return (
                    <li key={preset.country} className="ledger-row">
                      <Link
                        ...
                        className="group -mx-2 grid min-h-24 grid-cols-[2.25rem_minmax(0,1fr)_auto] gap-x-3 px-2 py-4 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent md:grid-cols-[2.5rem_minmax(9rem,0.56fr)_minmax(13rem,1fr)_7rem_2rem] md:items-center md:gap-x-6 md:py-5"
                      >
                        ...
                        <span aria-hidden="true" className="hidden text-xl text-accent transition-transform group-hover:translate-x-1 md:block">→</span>
                      </Link>
                    </li>
                  );
                })}
